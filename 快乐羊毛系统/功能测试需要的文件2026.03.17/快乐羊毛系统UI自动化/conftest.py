...
scope 说明：
  browser        session 级别 — 整个测试进程只启动一次浏览器
  page           module 级别  — 每个测试文件共享一个页面（不用每条用例都重新打开）
  logged_in_page module 级别  — 每个测试文件只登录一次
"""
import pytest
...
def page(browser):
    """每个测试文件共享一个页面上下文（不用每条用例都重新打开浏览器）"""
