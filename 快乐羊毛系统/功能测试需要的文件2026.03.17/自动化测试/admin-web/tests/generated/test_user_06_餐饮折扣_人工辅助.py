# -*- coding: utf-8 -*-
"""
user_06_餐饮折扣_人工辅助 · 自动化测试
共 47 条用例
来源模块: 餐饮折扣 （品牌会员共享模式二：人工辅助/验证码）, 餐饮折扣-人工辅助 (V3.0补充), 餐饮折扣-人工辅助 (复盘补漏), 餐饮折扣-人工辅助 (小程序环境)

运行方法:
  pytest admin-web/tests/generated\test_user_06_餐饮折扣_人工辅助.py -v --headed     # 有界面
  pytest admin-web/tests/generated\test_user_06_餐饮折扣_人工辅助.py -v              # 无头模式
  pytest admin-web/tests/generated\test_user_06_餐饮折扣_人工辅助.py -k "test_fyxt"  # 只跑编号含fyxt的
"""
import pytest
import sys
...
class Test餐饮折扣人工辅助:
    ...
    def test_cyzkrg_021(self, page):
        """[cyzkrg-021] 验证运营审核通过并输入验证码下发  [P1]"""
        # ── 前置条件 ──
    ...
    def test_cyzkrg_029(self, page):
        """[cyzkrg-029] 验证商家确认验证码后订单状态变为已完成  [P1]"""
        # ── 前置条件 ──
    ...
    def test_cyzkrg_014(self, page):
        """[cyzkrg-014] 验证不上传凭证直接提交的拦截 [反向]  [P2]"""
        # ── 前置条件 ──
    ...
    def test_cyzkrg_015(self, page):
        """[cyzkrg-015] 验证不输入金额直接提交的拦截 [反向]  [P2]"""
        # ── 前置条件 ──
        #   用户已上传凭证但未输入消费金额
        #
        # ── 测试步骤 ──
    ...
    def test_cyzkrg_022(self, page):
        """[cyzkrg-022] 验证验证码输入为空时的拦截 [反向]  [P2]"""
        # ── 前置条件 ──
        #   运营已点击"审核通过"，弹出验证码输入框
        #
        # ── 测试步骤 ──
    ...
    def test_cyzkrg_023(self, page):
        """[cyzkrg-023] 验证驳回时不输入理由的拦截 [反向]  [P2]"""
        # ── 前置条件 ──
        #   运营进入订单审核详情页
        #
        # ── 测试步骤 ──
    ...
    def test_cyzkrg_031(self, page):
        """[cyzkrg-031] 验证输入错误验证码核销失败 [反向]  [P2]"""
        # ── 前置条件 ──
        #   正确验证码为"668899"
        #
        # ── 测试步骤 ──
    ...
    def test_cyzkrg_033(self, page):
        """[cyzkrg-033] 验证订阅消息和短信均失败时的兜底处理 [反向]  [P2]"""
        # ── 前置条件 ──
    ...
    def test_cyzkrg_034(self, page):
        """[cyzkrg-034] 验证分佣计算在不同比例下的准确性  [P3]"""
        # ── 前置条件 ──
        #   分佣比例为15%，订单实付金额200元
        #
        # ── 测试步骤 ──
