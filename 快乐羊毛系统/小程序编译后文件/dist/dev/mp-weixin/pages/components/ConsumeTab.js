...
const _sfc_main = {
  __name: "ConsumeTab",
  setup(__props) {
    const consumeList = common_vendor.ref([]);
    ...
    const getConsumeList = async () => {
      ...
      try {
        const res = await api_my.consumptionRecord(params);
        if (res.data && res.data.data && res.data.data.length > 0) {
          consumeList.value = res.data.data;
        } else {
          consumeList.value = [];
        }
      } catch (error) {
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        m: consumeList.value.length > 0
      }, consumeList.value.length > 0 ? {
        n: common_vendor.f(consumeList.value, (item, k0, i0) => {
          return {
            a: common_vendor.t(item.goods_title),
            b: common_vendor.t(item.price),
            c: item.pay_price < 0 ? 1 : "",
            d: common_vendor.t(formatDate(item.create_time)),
            e: common_vendor.t(item.after_balance),
            f: item.id
          };
        })
      } : {});
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-149bf841"]]);
