...
function invokeSuccess(id, name, res) {
  ...
  return invokeCallback(id, extend$1(res || {}, result2));
}
...
function patch(instance, data, oldData) {
  ...
  if (mpType === "page" || mpType === "component") {
    ...
    const diffData = diff$1(data, getMPInstanceData(mpInstance, keys2));
...
function configFromStringAndFormat(config) {
  ...
  var string = "" + config._i, i, parsedInput, tokens2, token2, skipped, stringLength2 = string.length, totalParsedInputLength = 0, era, tokenLen;
  ...
  getParsingFlags(config).charsLeftOver = stringLength2 - totalParsedInputLength;
  if (string.length > 0) {
...
function splitString(text) {
  ...
  while (i < text.length) {
    ...
    if (DoubleUnicodePrefixReg.test(char) && DoubleUnicodeSuffixReg.test(text[i + 1])) {
      result2.push(text.substring(i, i + 2));
      i += 2;
    } else {
      result2.push(char);
      i += 1;
  ...
  match(text, surname) {
    ...
    for (let i = 0; i < zhChars.length; i++) {
      ...
      if (cur === null) {
        ...
        if (pattern) {
          result2.push(Object.assign(Object.assign({}, pattern), { index: i - pattern.length + 1 }));
        }
        ...
        while (failNode !== null) {
          ...
          if (pattern2) {
            result2.push(Object.assign(Object.assign({}, pattern2), { index: i - pattern2.length + 1 }));
          }
          failNode = failNode.fail;
        }
      }
...
var ERROR_CONF = { PARAM_ERR: 310, PARAM_ERR_MSG: "请求参数信息有误", SYSTEM_ERR: 600, SYSTEM_ERR_MSG: "系统错误", WX_ERR_CODE: 1e3, WX_OK_CODE: 200 };
...
var qqmapWxJssdk_min = QQMapWX$1;
const QQMapWX = /* @__PURE__ */ getDefaultExportFromCjs(qqmapWxJssdk_min);
