...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const handleAgent = async () => {
      const res = await api_endAgent.agentImg();
      console.log(res.data.agent_sub_img, "h火火恍恍惚惚");
      if (res && res.data.agent_sub_img) {
        bgImg.value = res.data.agent_sub_img;
      }
    };
    const handleAgentImg = async () => {
      const res = await api_endAgent.agentInvresellerQr();
      if (res) {
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-f6b3ebd3"]]);
